import csv
import json

DATASET_PATH = 'books-en.csv'
PATH_NAME = 'name.json'


def get_title(dataset):
    dataset.seek(0)
    title = next(dataset).strip().split(';')
    return [col.strip() for col in title]


def get_object(line, title):
    reader = csv.DictReader([line], title, delimiter=';', quotechar='"')
    return next(reader)


def find_by_name_filtered(dataset, title, name):
    results = []
    for line in dataset:
        obj = get_object(line, title)
        name_value = obj['Book-Title']
        if name.lower() in name_value.lower():
            results.append(obj)
    dataset.seek(0)
    return results


if __name__ == '__main__':
    with open(DATASET_PATH) as dataset:
        title = get_title(dataset)
        name_world = input("слово из названия (английский): ")
        res = find_by_name_filtered(dataset, title, name_world)
        with open(PATH_NAME, 'w') as out:
            json.dump(res, out, ensure_ascii=False, indent=4)
        dataset.seek(0)
        isbns = []
        for line in dataset:
            obj = get_object(line, title)
            isbns.append(obj['ISBN'])